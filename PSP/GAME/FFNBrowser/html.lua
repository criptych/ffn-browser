...
local _entities = {
   ...
   nbsp   = 0x00a0, copy   = 0x00a9, laquo  = 0x00ab, raquo  = 0x00bb,
   shy    = 0x00ad, reg    = 0x00ae, micro  = 0x00b5, frac14 = 0x00bc,
   ...
   ldquo  = 0x201c, rdquo  = 0x201d, hellip = 0x2026, prime  = 0x2032,
   Prime  = 0x2033, lsaquo = 0x2039, rsaquo = 0x203a, frasl  = 0x2044,
   euro   = 0x20ac, trade  = 0x2122, minus  = 0x2212, lowast = 0x2217,

   -- PGE 0.02 doesn't support XASCII, so replace some common chars
   -- with visually similar ASCII text

   [0x00a0] = ' ',    [0x00a9] = '(c)', [0x00ab] = '<<',  [0x00bb] = '>>',
   [0x00ad] = '',     [0x00ae] = '(R)', [0x00b5] = 'u',   [0x00bc] = '1/4',
   [0x00bd] = '1/2',  [0x00be] = '3/4',
   ...
   [0x201c] = '"',    [0x201d] = '"',   [0x2026] = '...', [0x2032] = '\'',
   [0x2033] = '"',    [0x2039] = '<',   [0x203a] = '>',   [0x2044] = '/',
   [0x2122] = '(tm)', [0x2212] = '-',   [0x2217] = '*',
}

function convert_entities(entity)
...
function convert_html(str, tagfunc)
   return (str
      :gsub('%s+', ' ')                      -- collapse whitespace
      :gsub("%b<>", tagfunc or convert_tags) -- strip HTML tags
      :gsub('^%s+', '')                      -- strip leading whitespace
      :gsub('[\128-\255]+', convert_utf8)    -- convert UTF-8 to ASCII
      :gsub("&(#?%w+);", convert_entities)   -- interpret entities
      )
end
